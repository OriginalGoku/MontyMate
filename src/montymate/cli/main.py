...
from pathlib import # spec_store.py
from __future__ import annotations
...
class SpecStorePaths:
    ...
    def run_root(self) -> Path:
        return self.repo_root / FACTORY_DIR_NAME / RUNS_DIR_NAME / self.run_id
...
class FileSpecStore:
    ...
    Layout:
      <repo_root>/.ai_module_factory/runs/<run_id>/
        module_spec.yaml
    ...
    def __init__(self, *, repo_root: str | Path, run_id: str):
        self.paths = SpecStorePaths(repo_root=Path(repo_root), run_id=run_id)
    ...
    def write_validation_report(self, report: Dict[str, Any]) -> None:
        self._ensure_dirs()
        self.paths.spec_validation_report_json.write_text(
